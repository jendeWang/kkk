  ...
  try {
      ...
      if (await dlg.isVisible()) {
        ...
        for (const t of tabs) {
          ...
          if (await tabEl.isVisible()) {
            ...
            const count = await page.locator('.el-dialog:visible [class*="card"], .el-dialog:visible .el-table__row').count();
            results.passed.push('产品-物模型"' + t + '"(' + count + '项)');
          }
        }
    ...
    if (await addBtn.isVisible()) {
      ...
      if (await dlg.isVisible()) {
        ...
        results.details['add_product_inputs'] = inputs;
        results.details['add_product_selects'] = selects;
    ...
    if (await devSelect.isVisible()) {
      await devSelect.click(); await page.waitForTimeout(800);
      const opts = await page.locator('.el-select-dropdown__item').allTextContents();
      results.details['telemetry_device_opts'] = opts;
      if (opts.length > 0) {
        results.passed.push('遥测-设备选项(' + opts.length + '个)');
    ...
    if (await propSelect.isVisible()) {
      await propSelect.click(); await page.waitForTimeout(800);
      const opts = await page.locator('.el-select-dropdown__item').allTextContents();
      results.details['telemetry_prop_opts'] = opts;
      if (opts.length > 0) {
        results.passed.push('遥测-属性选项(' + opts.length + '个)');
        await page.locator('.el-select-dropdown__item').first().click();
        await page.waitForTimeout(1500);
      }
    }
    ...
    // 时间范围选择
    const timePicker = page.locator('.el-date-picker, .el-time-picker, [class*="time"]').first();
    ...
    if (await cmdDevSelect.isVisible()) {
      await cmdDevSelect.click(); await page.waitForTimeout(800);
      const opts = await page.locator('.el-select-dropdown__item').allTextContents();
      results.details['command_device_opts'] = opts;
      if (opts.length > 0) {
    ...
    if (await svcSelect.isVisible()) {
      await svcSelect.click(); await page.waitForTimeout(800);
      const opts = await page.locator('.el-select-dropdown__item').allTextContents();
      results.details['command_service_opts'] = opts;
      if (opts.length > 0) {
        results.passed.push('命令-服务选项(' + opts.length + '个)');
        await page.locator('.el-select-dropdown__item').first().click();
        await page.waitForTimeout(1500);
      }
    }
    ...
    results.details['command_params'] = params;
    if (params > 0) {
    ...
    if (userRows > 0) {
      ...
      // 检查角色列
      const roles = await page.locator('.el-table__row .cell').allTextContents();
      if (roles.some(r => r.includes('管理员') || r.includes('操作员') || r.includes('查看员'))) {
        results.passed.push('用户-角色显示正确');
      }
    }
