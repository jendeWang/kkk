...
class SSEService:
    def __init__(self):
        ...
        self._command_clients: Set[asyncio.Queue] = set()
    ...
    async def add_command_client(self) -> asyncio.Queue:
        queue = asyncio.Queue()
        async with self._command_lock:
            self._command_clients.add(queue)
        return queue

    def remove_command_client(self, queue: asyncio.Queue):
        self._command_clients.discard(queue)
    ...
    async def publish_command_response(self, data: dict):
        message = f"event: command_response\ndata: {json.dumps({'type': 'command_response', 'data': data})}\n\n"
        async with self._command_lock:
            for q in list(self._command_clients):
                try:
                    await q.put(message)
                except Exception:
                    pass
...
async def command_sse_stream(request: Request):
    queue = await sse_service.add_command_client()
    try:
        yield f": keepalive session started\n\n"
        while True:
            if await request.is_disconnected():
        ...
        sse_service.remove_command_client(queue)
